...
before=c.execute('SELECT id,user_id,achievement_id,earned_at FROM user_achievements ORDER BY id').fetchall()
...
c.executescript(migration)
for p in sorted(pathlib.Path('migrations').glob('*.sql')):
 if p.name >= '0010':c.executescript(p.read_text())
assert before==c.execute('SELECT id,user_id,achievement_id,earned_at FROM user_achievements ORDER BY id').fetchall()
